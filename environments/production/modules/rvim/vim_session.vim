...
xmap ,e :NERDTreeToggle "quit
omap ,e :NERDTreeToggle "quit
nmap G Gzz
nmap N Nzz
...
silent only
cd ~/repos/puppet-lab/environments/production/modules/rvim
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +16 data/common.yaml
badd +1 hiera.yaml
badd +56 manifests/init.pp
badd +3 manifests/plugins.pp
badd +1 templates/vimrc.erb
badd +3 templates/vimrc_plugins.erb
badd +1 manifests/directories.pp
badd +0 manifests/config.pp
badd +21 metadata.json
argglobal
silent! argdel *
argadd data/common.yaml
argadd hiera.yaml
argadd manifests/init.pp
argadd manifests/plugins.pp
argadd manifests/config.pp
argadd templates/vimrc.erb
argadd templates/vimrc_plugins.erb
...
inoremap <buffer> <silent> Ý =AutoPairsMoveCharacter(']')
inoremap <buffer> <silent> Û =AutoPairsMoveCharacter('[')
nmap <buffer> \= <Plug>AlignRange
xmap <buffer> \= <Plug>AlignRange
omap <buffer> \= <Plug>AlignRange
...
silent! normal! zE
let s:l = 56 - ((24 * winheight(0) + 22) / 44)
